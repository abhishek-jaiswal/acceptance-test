...
describe('Cross Origin Requests', function() {
  ...
  it('should return the content type : content-type', function() {

    return assert(result, "headers").to.have.deep.property('content-type')
  });

  it('should allow same origins: SAMEORIGIN', function() {
    return assert(result, "header.x-frame-options").to.equal('SAMEORIGIN');
  });

  it('should return the correct content type: application/json', function() {
    return assert(result, "header.content-type").to.equal('application/json');
  });
});
